...
**Result: 337 passed | 0 failed | 17 skipped (live-only)**
**Duration: ~2.0s total**

| File | Tests | Duration | Description |
|------|------:|----------|-------------|
| `src/store.test.ts` | 61 | 104ms | InMemoryStateStore CRUD, concurrency, TTL, listByPhase, `listExpired` phase-filter |
| `src/settler.test.ts` | 45 | 570ms | Broadcast, polling, settlement, timeout, deposit-notify, **recovery** |
| `src/verifier.test.ts` | 34 | 309ms | EIP-712 signature verification (EIP-3009 + Permit2) |
| `src/quote-engine.test.ts` | 40 | 14ms | 1CS quote translation to x402 PaymentRequirements, recipient/asset diagnosis, error-context threading |
| `src/rate-limiter.test.ts` | 25 | 155ms | Per-IP quote rate limiting, settlement limiter, quote GC (never deletes in-flight) |
...
| `src/middleware.test.ts` | 19 | 189ms | Express middleware request/response handling + client-facing error sanitization + correlation IDs + server-side context logging |
| `src/client/signer.test.ts` | 12 | 42ms | EIP-3009 and Permit2 signing, chain ID extraction |
| `src/config.test.ts` | 19 | 13ms | Zod schema validation, env var parsing, CORS allowlist parsing, recipient-format warnings |
...
- Actual test execution: ~2.0s for 337 tests (~5.9ms per test average)
- Slowest file: `src/settler.test.ts` (570ms) — includes recovery tests with async background tasks
...
|----------|------:|----------:|---------------:|
| Unit + Integration (mocked) | 337 | 100% | ~2.0s |
